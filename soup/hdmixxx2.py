...
def List(url,f):
    ...
    try:
		...
		match2 = re.compile('<div class="navigation" align="center" ><span>(.+?)</span> <span>(.+?)</span> <a href="(.+?)">(.+?)</a>',          re.DOTALL).findall(listhtml)
		if match2:
			print match2[0][0]
			print match2[0][1]
			print match2[0][2]
...
print("... start");
f = open(os.environ['HOME']+'/hdmixxxip.m3u8', 'w')
f.write('#EXTM3U\n')
#List('https://www.hdmi-tv.ru/regional/',f);
