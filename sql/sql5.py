...
def clear_db():
	c.execute('DELETE FROM BaseDeDados')
	conn.commit()
		   
def del_and_update():
	show_db()
	c.execute('UPDATE BaseDeDados SET value = 99 WHERE value=8')
	conn.commit()
	c.execute('DELETE FROM BaseDeDados WHERE value=99')
	conn.commit()
	show_db()
	c.execute('SELECT * FROM BaseDeDados WHERE value=2')
